...
const SellerNotifiedRequests = () => {
  ...
  const fetchNotifiedRequests = async () => {
    ...
    try {
      ...
      const { data } = await axios.get('http://16.171.138.104:5000/api/requests/notified', {
  ...
  const fetchSubmittedOffers = async () => {
    try {
      const token = localStorage.getItem('token');
      const { data } = await axios.get('http://16.171.138.104:5000/api/offers/seller/my-offers', {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const handleSubmitOffer = async () => {
    try {
      const token = localStorage.getItem('token');
      await axios.post(
        `http://16.171.138.104:5000/api/offers/${selectedRequest._id}`,
        offerData,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
